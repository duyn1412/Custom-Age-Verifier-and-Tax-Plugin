 ...
 * Plugin URI: https://github.com/duyn1412/Custom-Age-Verifier-and-Tax-Plugin
 ...
 * Text Domain: custom-age-verifier-tax
...
function ba_get_current_province() {
    ...
    $val = null;
    // Get checkout province from form data instead of session
    $checkout_province = '';
    if (isset($_POST['shipping_state']) && $_POST['shipping_state'] !== '') {
        $checkout_province = sanitize_text_field(wp_unslash($_POST['shipping_state']));
    } elseif (isset($_POST['state']) && $_POST['state'] !== '') {
        $checkout_province = sanitize_text_field(wp_unslash($_POST['state']));
    }

    // echo "<br>DEBUG ba_get_current_province():<br>";
    // echo "checkout_province: " . ($checkout_province ? $checkout_province : 'null') . "<br>";
    // echo "_GET[province]: " . (isset($_GET['province']) ? $_GET['province'] : 'not set') . "<br>";
    // echo "_COOKIE[province]: " . (isset($_COOKIE['province']) ? $_COOKIE['province'] : 'not set') . "<br>";
    ...
    // Check if this is a checkout AJAX request
    $is_checkout_ajax = isset($_GET['wc-ajax']) && $_GET['wc-ajax'] === 'update_order_review';
    ...
    // Debug checkout detection
    // ba_log("ba_get_current_province(): CHECKOUT DETECTION - is_checkout(): " . (is_checkout() ? 'true' : 'false') . 
    //        ", is_page('checkout'): " . (is_page('checkout') ? 'true' : 'false') . 
    //        ", current_id: " . $current_id . 
    //        ", checkout_id: " . $checkout_id . 
    //        ", global_post_id: " . $global_post_id . 
    //        ", uri_contains_checkout: " . ($uri_contains_checkout ? 'true' : 'false') . 
    //        ", is_checkout_ajax: " . ($is_checkout_ajax ? 'true' : 'false') . 
    //        ", REQUEST_URI: " . $_SERVER['REQUEST_URI']);
    
    // echo "is_checkout_page: " . ($is_checkout_page ? 'true' : 'false') . "<br>";
    
    // PRIORITY 1: Checkout form province (from form data) - HIGHEST PRIORITY on checkout page
    if ($is_checkout_page && function_exists('WC') && WC()->session) {
        // ba_log("ba_get_current_province(): PRIORITY 1 - is_checkout_page: true, checkout_province: " . ($checkout_province ? $checkout_province : 'null'));
        if ($checkout_province && ba_is_valid_province_code($checkout_province)) {
            // ba_log("ba_get_current_province(): RETURNING checkout_province = " . strtoupper($checkout_province));
            $cached = true;
            $cached_province = strtoupper($checkout_province);
            return strtoupper($checkout_province);
        } else {
            // ba_log("ba_get_current_province(): PRIORITY 1 - checkout_province invalid or null");
        }
    } else {
        // ba_log("ba_get_current_province(): PRIORITY 1 - is_checkout_page: " . ($is_checkout_page ? 'true' : 'false') . ", WC session: " . (function_exists('WC') && WC()->session ? 'true' : 'false'));
    }
    
    // PRIORITY 2: GET parameter (query string) - HIGHEST PRIORITY on non-checkout pages
    // ba_log("ba_get_current_province(): PRIORITY 2 - is_checkout_page: " . ($is_checkout_page ? 'true' : 'false') . ", GET province: " . (isset($_GET['province']) ? $_GET['province'] : 'not set'));
    if (!$is_checkout_page && isset($_GET['province']) && is_string($_GET['province'])) {
        $val = strtoupper(sanitize_text_field($_GET['province']));
        if (!ba_is_valid_province_code($val)) { 
            // ba_log("ba_get_current_province(): RETURNING null (invalid GET province)");
            $cached = true;
            $cached_province = null;
            return null; 
        }
        // ba_log("ba_get_current_province(): RETURNING GET province = " . $val);
        $cached = true;
        $cached_province = $val;
        return $val;
    } else {
        // ba_log("ba_get_current_province(): PRIORITY 2 - SKIPPED (is_checkout_page: " . ($is_checkout_page ? 'true' : 'false') . ", GET province: " . (isset($_GET['province']) ? $_GET['province'] : 'not set') . ")");
    }
    
    // PRIORITY 3: Cookie (fallback) - only if no session on checkout page
    if (isset($_COOKIE['province']) && is_string($_COOKIE['province'])) {
        // Skip cookie if we have session on checkout page
        if ($is_checkout_page && $checkout_province) {
            // ba_log("ba_get_current_province(): SKIPPING cookie (have session on checkout)");
        } else {
            $val = strtoupper(sanitize_text_field($_COOKIE['province']));
            if (!ba_is_valid_province_code($val)) { 
                // ba_log("ba_get_current_province(): RETURNING null (invalid cookie province)");
                $cached = true;
                $cached_province = null;
                return null; 
            }
            // ba_log("ba_get_current_province(): RETURNING cookie province = " . $val);
            $cached = true;
            $cached_province = $val;
            return $val;
        }
...
add_action('template_redirect', function () {
    ...
    // Skip redirect on checkout page to avoid conflicts with session
    if (is_checkout() || is_page('checkout') || (get_the_ID() == get_option('woocommerce_checkout_page_id'))) return;
...
add_action('woocommerce_before_calculate_totals', function($cart) {
    ...
    if ($running) {
        echo "<br>DEBUG: woocommerce_before_calculate_totals - PREVENTED LOOP<br>";
        return;
    }
    
    echo "<br>DEBUG: woocommerce_before_calculate_totals - START<br>";
    echo "Cart items count: " . $cart->get_cart_contents_count() . "<br>";
    ...
    try {
        ...
        $running = false;
        echo "<br>DEBUG: woocommerce_before_calculate_totals - END<br>";
    }
...
add_action('wp_ajax_cfw_update_cart', function() {
    echo "<br>DEBUG: cfw_update_cart AJAX - START<br>";
    $province = ba_get_current_province();
    echo "DEBUG: cfw_update_cart province: " . ($province ? $province : 'null') . "<br>";
    if ($province) {
        $_GET['province'] = $province;
        echo "DEBUG: cfw_update_cart set _GET[province] = " . $province . "<br>";
    }
    echo "DEBUG: cfw_update_cart - END<br>";
}, 0);

add_action('wp_ajax_nopriv_cfw_update_cart', function() {
    echo "<br>DEBUG: cfw_update_cart AJAX (nopriv) - START<br>";
    $province = ba_get_current_province();
    echo "DEBUG: cfw_update_cart (nopriv) province: " . ($province ? $province : 'null') . "<br>";
    if ($province) {
        $_GET['province'] = $province;
        echo "DEBUG: cfw_update_cart (nopriv) set _GET[province] = " . $province . "<br>";
    }
    echo "DEBUG: cfw_update_cart (nopriv) - END<br>";
}, 0);
...
add_action('woocommerce_checkout_update_order_review', function () {
    ...
    if ($state && function_exists('WC') && WC()->session) {
        // ba_log("woocommerce_checkout_update_order_review: BEFORE setting - current session: " . WC()->session->get('ba_checkout_province'));
        WC()->session->set('ba_checkout_province', strtoupper($state));
        // Sync cookie with session to maintain consistency
        //ba_set_province_cookie(strtoupper($state));
        ba_log("woocommerce_checkout_update_order_review: AFTER setting - new session: " . WC()->session->get('ba_checkout_province'));
    }
}, 5); // HIGH PRIORITY - chạy trước woocommerce_before_calculate_totals
